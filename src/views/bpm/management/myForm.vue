/** *Copyright: Copyright (c) 2023 *Author:Leo *Version 1.0 *Title: form-generator/Element
Plus 表单解析器 *GitHub: https://github.com/leo.one.tt/aibpm */
...
<template>
  ...
  <template v-if="state.isShow">
    <el-drawer v-model="state.isShow" direction="rtl" :size="state.drawerSize">
      ...
      >
        ...
        <el-tab-pane name="record" label="审批记录" v-if="state.conf.items.length > 0">
          <tempate>
            <el-timeline style="font-size: 0.8em">
              ...
              >
                ...
                <p>
                  处理结果：
                  <el-tag :type="getType(activity)"> {{ getState(activity) }}</el-tag>
                </p>

                <p v-if="!!activity.comment">处理意见：{{ activity.comment }}</p>
              </el-timeline-item>
            </el-timeline>
          </tempate></el-tab-pane
        >
        <el-tab-pane name="history" label="审批历史" v-if="state.conf.items.length > 0">
          <tempate>
            <el-table :data="state.conf.items" style="font-size: 0.9em">
              ...
              >
                <template #default="{ row }">
                  <span v-show="dayjs(row.finishTime).year() > 2020">{{
                    formatterTimeValue(row.finishTime)
                  }}</span>
                </template>
              </el-table-column>
              <el-table-column
              ...
              >
                <template #default="{ row }">
                  <el-tag
                    disable-transitions
                    :type="vnDisplayType[row.state & 127].toLowerCase()"
                    >{{ vnActivityState[row.state] }}</el-tag
                  >
                </template>
              </el-table-column>
              <el-table-column
      ...
      <template #footer>
        <div
          style="flex: auto; background-color: #eeeeee; padding: 5px 5px 5px 5px"
          v-if="state.conf.activity.type"
        >
          <el-button
            type="warning"
            v-if="state.conf.activity.type !== 'Copy'"
            plain
            @click="submitForm({ isSaveOnly: true })"
            >保存</el-button
          >
          <el-button
            type="success"
            v-if="['Start', 'Deal'].includes(state.conf.activity.type)"
            :loading="state.submitLoading"
            plain
            @click="submitForm()"
            >提交</el-button
          >
          <el-button type="danger" v-if="false" plain @click="openApprove()"
            >驳回</el-button
          >
          <el-button
            type="success"
            v-if="state.conf.activity.type === 'Approve'"
            plain
            @click="openApprove()"
            >审批</el-button
          >
  ...
  >
  <ApprovePanel
    :approve-setting="state.conf.approveSetting"
    ref="approvePanelRef"
  ></ApprovePanel>
